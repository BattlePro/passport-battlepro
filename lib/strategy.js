 ...
 * @property {string} [domain="https://battlepro.com"]
 * @property {string} [authorizationURL="https://battlepro.com/oauth2/authorize"]
 * @property {string} [tokenURL="https://battlepro.com/oauth2/token"]
 * @property {string} [scopeSeparator=" "]
 */
/**
 * `Strategy` constructor.
 *
 * The Battlepro authentication strategy authenticates requests by delegating to
 * Battlepro via the OAuth2.0 protocol
 *
 ...
 *
 * Options:
 *   - `clientID`       OAuth ID to battlepro
 *   - `clientSecret`   OAuth Secret to verify client to battlepro
 *   - `callbackURL`    URL that battlepro will redirect to after auth
...
function Strategy(options, verify) {
    options = options || {};
    domain = options.domain || 'https://battlepro.com';
    options.authorizationURL = options.authorizationURL || (domain + '/oauth2/authorize');
    options.tokenURL = options.tokenURL || (domain + '/oauth2/token');
...
/**
 * Retrieve user profile from Battlepro.
 *
...
Strategy.prototype.userProfile = function(accessToken, done) {
    var self = this;
    this._oauth2.get(domain + '/oauth2/me', accessToken, function(err, body, res) {
        if (err) {
...
Strategy.prototype.checkScope = function(scope, accessToken, cb) {
    if (this._scope && this._scope.indexOf(scope) !== -1) {
        this._oauth2.get(domain + '/oauth2/me/' + scope, accessToken, function(err, body, res) {
